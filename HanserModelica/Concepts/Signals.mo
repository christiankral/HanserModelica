...
model Signals "Application of signals, sources, sesors"
  ...
  Modelica.Blocks.Sources.Step step(offset=0,height=IRef,startTime=1) annotation (Placement(transformation(extent={{-90,-10},{-70,10}})));
  Modelica.Blocks.Math.Feedback feedback annotation (Placement(transformation(extent={{-60,-10},{-40,10}})));
  Modelica.Blocks.Continuous.Integrator integrator(k=100) annotation (Placement(transformation(extent={{-30,-10},{-10,10}})));
equation
  connect(ground.p, signalVoltage.n) annotation (Line(points={{10,-20},{10,-10}},
                                                                                color={0,0,255}));
  connect(signalVoltage.p, resistor.p) annotation (Line(points={{10,10},{10,20},{30,20}},
                                                                                        color={0,0,255}));
