...
model SMR_CurrentSource "Synchronous reluctance machine fed by current source"
  ...
  Modelica.Mechanics.Rotational.Sources.QuadraticSpeedDependentTorque quadraticTorqueQS(
    TorqueDirection=false,
    tau_nominal=-TLoad,
    w_nominal(displayUnit="rpm") = wNominal) annotation (Placement(transformation(extent={{100,0},{80,20}})));
  ...
  Modelica.Electrical.QuasiStationary.SinglePhase.Basic.Ground
    groundMQS annotation (Placement(transformation(
        ...
        rotation=0,
        origin={-60,0})));
  Modelica.Magnetic.QuasiStatic.FundamentalWave.Utilities.MultiTerminalBox terminalBoxQS(terminalConnection="Y", m=m) annotation (Placement(transformation(extent={{-10,16},{10,36}})));
  Modelica.Magnetic.QuasiStatic.FundamentalWave.Utilities.CurrentController currentControllerQS(m=m, p=smrQS.p) annotation (Placement(transformation(extent={{-50,70},{-30,90}})));
  ...
  Modelica.Magnetic.QuasiStatic.FundamentalWave.BasicMachines.SynchronousMachines.SM_ReluctanceRotor smrQS(
    ...
    TrOperational=293.15) annotation (Placement(transformation(extent={{-10,0},{10,20}})));
...
equation
  ...
  connect(groundMQS.pin, starMachineQS.pin_n) annotation (Line(
      points={{-60,10},{-40,10}},
      color={85,170,255}));
  connect(currentControllerQS.I, referenceCurrentSource.I) annotation (Line(points={{-29,84},{-20,84},{-20,86},{-12,86}},
                                                                                                        color={85,170,255}));
  ...
  connect(starQS.pin_n, groundeQS.pin) annotation (Line(
      points={{50,70},{50,70}},
      color={85,170,255}));
  connect(referenceCurrentSource.plug_p, resistorQS.plug_p) annotation (Line(points={{0,90},{20,90}},   color={85,170,255}));
  connect(resistorQS.plug_n, referenceCurrentSource.plug_n) annotation (Line(points={{20,70},{0,70}}, color={85,170,255}));
  ...
  connect(smrQS.flange, rotorAngleQS.flange) annotation (Line(points={{10,10},{20,10}}, color={0,0,0}));
  connect(smrQS.plug_sp, rotorAngleQS.plug_p) annotation (Line(points={{6,20},{24,20}}, color={85,170,255}));
  connect(smrQS.plug_sn, rotorAngleQS.plug_n) annotation (Line(points={{-6,20},{-6,26},{36,26},{36,20}}, color={85,170,255}));
  connect(inertiaLoadQS.flange_a, smrQS.flange) annotation (Line(points={{50,10},{10,10}},                                 color={0,0,0}));
  connect(currentRMSSensorQS.plug_p, referenceCurrentSource.plug_n) annotation (Line(points={{0,60},{0,70}}, color={85,170,255}));
  connect(currentRMSSensorQS.plug_n, terminalBoxQS.plugSupply) annotation (Line(points={{0,40},{0,22}}, color={85,170,255}));
  ...
  connect(currentControllerQS.gamma, referenceCurrentSource.gamma) annotation (Line(points={{-29,76},{-20,76},{-20,74},{-12,74}},
                                                                                                                color={0,0,127}));
  ...
  connect(voltageRMSSensorQS.plug_n, currentRMSSensorQS.plug_n) annotation (Line(points={{-20,40},{-6.66134e-16,40}}, color={85,170,255}));
  connect(starMQS.pin_n, groundMQS.pin) annotation (Line(points={{-50,20},{-50,10},{-60,10}}, color={85,170,255}));
  annotation (
