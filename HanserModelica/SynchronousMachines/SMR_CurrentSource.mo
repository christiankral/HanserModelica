...
model SMR_CurrentSource "Synchronous reluctance machine fed by current source"
  ...
  Modelica.Mechanics.Rotational.Components.Inertia inertiaLoad(J = 0.29) annotation (
    Placement(transformation(extent = {{50, 0}, {70, 20}})));
  Modelica.Mechanics.Rotational.Sources.QuadraticSpeedDependentTorque quadraticTorque(TorqueDirection = false, tau_nominal = -TLoad, w_nominal(displayUnit = "rpm") = wNominal) annotation (
  ...
  parameter Modelica.Electrical.Machines.Utilities.ParameterRecords.SM_ReluctanceRotorData smrData(Lmd = 2.9 / (2 * pi * smrData.fsNominal), Lmq = 0.36 / (2 * pi * smrData.fsNominal), useDamperCage = false,
    effectiveStatorTurns=64)                                                                                                                                                                                   annotation (
...
equation
  connect(quadraticTorque.flange, inertiaLoad.flange_b) annotation (
  ...
  connect(smr.plug_sp, rotorAngle.plug_p) annotation (
    Line(points = {{6, 20}, {24, 20}}, color = {85, 170, 255}));
  connect(smr.plug_sn, rotorAngle.plug_n) annotation (
    Line(points = {{-6, 20}, {-6, 26}, {36, 26}, {36, 20}}, color = {85, 170, 255}));
  connect(inertiaLoad.flange_a, smr.flange) annotation (
  ...
  connect(id.y, currentController.id_rms) annotation (
    Line(points = {{-69, 80}, {-60, 80}, {-60, 86}, {-52, 86}}, color = {0, 0, 127}));
  connect(iq.y, currentController.iq_rms) annotation (
  ...
  connect(starM.pin_n, groundM.pin) annotation (
...
<p>
This example investigates a quasi static model of a synchronous reluctance machine. 
...
<li><code>smpm.tauElectrical</code>: machine torque</li>
    ...
    Diagram(coordinateSystem(preserveAspectRatio = false, extent = {{-100, -100}, {100, 100}}), graphics={  Text(extent = {{30, 38}, {110, 30}}, textStyle={TextStyle.Bold},                            lineColor={0,0,0},
