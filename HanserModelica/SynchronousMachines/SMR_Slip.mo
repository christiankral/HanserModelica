...
model SMR_Slip "Synchronous reluctance machine operated at small slip"
  ...
  Modelica.Magnetic.QuasiStatic.FundamentalWave.BasicMachines.SynchronousMachines.SM_ReluctanceRotor smrQS(
    ...
    TrOperational=373.15) annotation (Placement(transformation(extent={{-10,20},{10,40}})));
  ...
  Modelica.Electrical.QuasiStationary.MultiPhase.Sources.VoltageSource
    ...
    phi=-Modelica.Electrical.MultiPhase.Functions.symmetricOrientation(
        m),
    V=fill(VsNominal, m),
    f=fNominal) annotation (Placement(transformation(
        origin={-30,80},
        extent={{-10,-10},{10,10}},
        rotation=180)));
...
equation
  ...
  connect(terminalBoxQS.plug_sn, smrQS.plug_sn) annotation (Line(points={{-6,40},{-6,40}}, color={85,170,255}));
  connect(terminalBoxQS.plug_sp, smrQS.plug_sp) annotation (Line(points={{6,40},{6,40}}, color={85,170,255}));
  ...
  connect(smrQS.flange, rotorAngleQS.flange) annotation (Line(points={{10,30},{20,30}}, color={0,0,0}));
  connect(smrQS.flange, mechanicalPowerSensorQS.flange_a) annotation (Line(points={{10,30},{50,30}}, color={0,0,0}));
